$(document).ready(function(){
  ...
  var table_users = $('#table_users').dataTable({
    "ajax": base_url + "/services/users.php?job=get_users",
    "columns": [
      { "data": "id" },
  ...
  $(document).on('submit', '#form_user.add', function(e){
    ...
    if (form_user.valid() == true){
      ...
      var request   = $.ajax({
        url:          base_url + '/services/users.php?job=add_user',
        cache:        false,
        data:         form_data,
        dataType:     'json',
  ...
  $(document).on('click', '.function_edit a', function(e){
    ...
    var id      = $(this).data('id');
    var request = $.ajax({
      url:          base_url + '/services/users.php?job=get_user',
      cache:        false,
      data:         'id=' + id,
      dataType:     'json',
      contentType:  'application/json; charset=utf-8',
  ...
  $(document).on('submit', '#form_user.edit', function(e){
    ...
    if (form_user.valid() == true){
      ...
      var form_data = $('#form_user').serialize();
      var request   = $.ajax({
        url:          base_url + '/services/users.php?job=edit_user&id=' + id,
        cache:        false,
        data:         form_data,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
  ...
  $(document).on('click', '.function_delete a', function(e){
    ...
    if (confirm("Are you sure you want to delete '" + username + "'?")){
      ...
      var id      = $(this).data('id');
      var request = $.ajax({
        url:          base_url + '/services/users.php?job=delete_user&id=' + id,
        cache:        false,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
        type:         'get'
